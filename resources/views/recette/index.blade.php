...
<body>
  <nav class="relative select-none bg-gray-200 lg:flex lg:items-stretch w-full">
    ...
    <div class="lg:flex lg:items-stretch lg:justify-end ml-auto">
        <button class="group relative mx-2 md:mx-4 my-3 h-8 md:w-40 overflow-hidden rounded-lg bg-white text-sm shadow">
...
<div class="min-h-screen  flex justify-center items-center py-20">
    <div class="md:px-2 md:grid md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-5 space-y-4 md:space-y-0">
        @foreach($recette as $rec)
        <div class="max-w-sm bg-gray-100 px-2 md:px-6 pt-6 pb-2 rounded-xl shadow-lg transform hover:scale-105 transition duration-500">
            <h3 class="mb-2 md:mb-3 text-xl font-bold text-indigo-600">{{$rec['title']}}</h3>
            <div class="relative">
                <img class="w-full rounded-xl" src="{{ asset('storage/image/' . $rec['picture']) }}" alt="Image de Recette" />
            </div>
            <p class="mt-2 md:mt-4 text-gray-800 whitespace-pre-line   cursor-pointer">{{$rec['description']}}</p>
            <div class="my-4">
                <button class="mt-2 md:mt-4 text-xl w-full text-white bg-amber-400 py-2 rounded-xl shadow-lg"><a href="{{ route('recette.show', ['recette' => $rec['id']]) }}">Show more</a></button>
            </div>
        </div>
        @endforeach
    </div>
